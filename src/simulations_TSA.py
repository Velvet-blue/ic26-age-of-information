...
def evolution_AoI_sim(arrival_prob, send_prob, threshold, time=100):
    ...
    for t in range(time):
        ...
        A_tries = A_have and sends_A[t] and current_aoi_A > threshold
        B_tries = B_have and sends_B[t] and current_aoi_B > threshold
        # não é >= pois já estamos no tempo t+1, depois do update
...
def evolution_AoI_til_sim(arrival_prob, send_prob, threshold, time=100):
    ...
    for t in range(time):
        ...
        ev_aoi_A[t] = current_aoi_A - tau_A

        AAoI += current_aoi_A
        current_aoi_A += 1
        current_aoi_B += 1

    return ev_aoi_A, ev_aoi_B, ev_tau_A, np.arange(time)[arrivals_A], updates[:successes], PAoI/successes, AAoI/time, successes
...
def evolution_AoI_plinio_sim(arrival_prob, send_prob, threshold, time=100):
    ...
    return ev_aoi_A, ev_aoi_B, arrivals_succes_pack[:successes], updates[:successes], PAoI/successes, AAoI/time, successes, arrivals_times
